...
void TestRotatedBoxMatchesAnalyticBound() {
  ...
  // The bound this replaced was aabb_radius on every axis, including the thin
  // one.
  EXPECT_LT(expected[2], box->aabb_radius / S(100));
}
...
// Whatever the pose, the bound must contain the geometry and must never be
// looser than the looser cube of half-width aabb_radius.
template <typename S>
void TestBoundIsConservative() {
  ...
  for (const PosedBox<S>& test_case : cases) {
    ...
    const Vector3<S> bound_half_widths = (aabb.max_ - aabb.min_) * S(0.5);
    EXPECT_TRUE(
        (bound_half_widths.array() <= box->aabb_radius + tolerance).all());
  }
}
...
GTEST_TEST(CollisionObjectAABB, BoundIsConservative) {
  TestBoundIsConservative<double>();
  TestBoundIsConservative<float>();
}
